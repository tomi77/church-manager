...
The engine layer is split into **data classes** (resources/data containers — `Religion`, `Province`, `Faction`, `War`, `RelationState`, `Coalition`, `MissionaryMission`, `Idea`, `DefeatEvent`, `ProvinceGraph`) and **stateless Manager classes** (`TurnManager`, `WarManager`, `DiplomacyManager`, `DoctrineManager`, `SchismManager`). Managers extend `RefCounted`, hold no state, and are instantiated per call (`var wm := WarManager.new()`). They take `state: Node` (GameState) as their first parameter and mutate it.

- **`TurnManager.process_turn(state)`** — orchestrates the end-of-turn pipeline in fixed order: passive pressure → holy site prestige → faction tensions → scholar missions → believer exodus → wars → missionaries → diplomacy → resources → vassal revolts → `state.advance_turn()`. Loaders (`ReligionLoader`, `ProvinceLoader`) parse JSON fixtures from `data/` (e.g. `provinces_historical.json` — 12 provinces with hand-authored `position{x,y}` for map rendering).
...
- **`MainShell`** (`scripts/ui/MainShell.gd` + `.tscn`) is the root Control. Contains `Header` (player info + End Turn) + `TabBar` + 4 tabs in `ContentArea`: `MapaTab` (Plan 09), `WiaraTab` (placeholder), `SwiatTab` (diplomacy, Plan 08), `FrakcjeTab` (placeholder). Tab visibility flips via `_on_tab_changed`.
- **Cross-tab navigation:** `MapaTab` emits `navigate_to_diplomacy(religion_id)` → MainShell switches tab to `swiat` and calls `WorldTab.preselect_religion(id)`.
- **Refresh model:** every component has `bind_state(state)` and `refresh()`. After engine mutations, the chain `state_changed` signal → `MainShell.refresh()` → tab refresh causes a full rerender (no dirty-tracking). MapaTab's `ProvinceDetailPanel` additionally refreshes itself on `war_declared` / `missionaries_sent` for immediate feedback.
