...
func test_dialog_shifts_axis_toward_faction_preference() -> void:
	...
	var gs := _make_state()
	var rel: Religion = gs.get_religion("islam")
	var faction := rel.factions[0]	# ulama: axis_preferences = [{axis: A, direction: 1}, {axis: B, direction: 1}]
	faction.tension = 60.0
	assert_true(faction.axis_preferences.size() > 0, "Test wymaga frakcji z axis_preferences — ulama powinno je mieć")
