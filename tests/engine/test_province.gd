...
func test_province_get_pressure_returns_stored_value() -> void:
	var p := Province.new()
	p.pressure = {"islam": 45.0, "western_christianity": 20.0}
	assert_eq(p.get_pressure("islam"), 45.0)
	assert_eq(p.get_pressure("western_christianity"), 20.0)
...
func test_province_dominant_pressure_returns_religion_with_highest_pressure() -> void:
	...
	p.pressure = {"islam": 72.0, "western_christianity": 80.0, "judaism": 10.0}
	assert_eq(p.dominant_pressure_religion(), "western_christianity")
